...
yield <- function(low, high, f){
  ...
  f.vec  <- rep(f,12)
  ofl    <- f*bio
  buffer <- 0.75 #Value used to make plotting better
  abc    <- ofl*buffer
...
png(filename = "figures/HCRs.png", width = 6.7, height = 6, units = 'in', res = 256)
...
out <- yield(low, high, f); z1 = out$z1 ; z2 = out$z2
plot(seq(high,1, step), out$ofl[z2:length(out$ofl)],,type='l',ylim=c(0, 20),xlim=c(0,0.50),
     axes=F,ylab="",xlab="",xaxs="i",yaxs="i",lwd=2,cex.axis=1.2, col = "white")
box()

lines(seq(0,1,step), out$ofl, lty = 1, lwd = 2, col = "white")
# abc
lines(seq(high,1,step), out$abc[z2:length(out$abc)], lty = 1, lwd = 1, col = "white")
lines(seq(0,1,step), out$abc, lty = 1, lwd = 2, col = "white")
lines(rep(low, 2), c(0, 20), lty = 3, col = t.col)
lines(rep(high, 2), c(0, 20), lty = 3, col = t.col)
#lines(rep(high/2, 2), c(0, 20), lty = 3, col = t.col)
# acl 
...
print.letter(label = "30-10", xy = c(0.10, 0.92), cex = 1)
...
print.letter(label = "40-10", xy = c(0.10, 0.92), cex = 1)
